...
public class AdminGUI {
	...
	private JPanel pnlUserSettingsBtns = new JPanel(new BorderLayout());
	...
	*/
	public AdminGUI(ListenForClients server) {
		...
		panel2.add(btnResetPW);
		panel2.add(btnRemoveUser);
		panel2.add(btnUpdate);
		pnlUserSettingsBtns.add(panel2, BorderLayout.NORTH);
		pnlUserSettingsBtns.add(txtAreaUser, BorderLayout.CENTER);
	...
	private class ButtonListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btnRemoveUser) {
				showFrame(false);
				new VerifyRemove(gui, admin, list.getSelectedIndex());
...
//				list.setModel(admin.getModel());
			...
			if (e.getSource() == btnResetPW) {
				showFrame(false);
				new VerifyReset(gui ,admin, list.getSelectedIndex());
